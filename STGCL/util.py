...
'''
  index: index for target batch
  start_times: list of start time for batched
  r_f: threshold
'''
def negativeFilter(index, start_times, r_f):
  time_diff = np.round(r_f/24/60, 4)
  time = start_times[index]
  output = []
  for i in range(len(start_times)):
    if abs(start_times[i] - time) < time_diff:
      output.append(i)
  return output
...
def c_loss(device, x1, x2, start_times, r_f=30):
  ...
  for i in range(batch_size):
    ...
    for nega in nega_samples:
        nega_sim[i][nega] = sim_matrix[i][nega]
    # print(i)
    # print(nega_samples)
